...
function valueMenu(){
	document.getElementById('menu').innerHTML = '"Your favorite items."';
}
...
function randQuote(){
	var rInt = Math.floor(Math.random()*quotes.length);
	document.getElementById("displayQuotes").innerHTML = quotes[rInt];
}

document.getElementById("random").addEventListener("click",randQuote);

//9. Unlock the Secret to Financial Freedom
/*Create an event listener that will show and hide the message when clicking on the button.
*/

var theMoney = document.getElementById('showmoney').innerHTML;
var buttonLoc = document.getElementById("showHide");


var theFree = document.getElementById("freedom");
var rMoney = document.createElement("div");
rMoney.id = 'realMoney';
buttonLoc.after(rMoney);
...
function moMoney(){
	...
	if (moneySwitch == 0){
		document.getElementById('realMoney').innerHTML = theMoney;
		moneySwitch++;
	}
	else if (moneySwitch == 1){
		document.getElementById('realMoney').innerHTML = "";
		moneySwitch--;
	}
}

document.getElementById("showHide").addEventListener("click", moMoney);
